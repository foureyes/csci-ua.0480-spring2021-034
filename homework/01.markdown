...
* some emoji take up more than one "character width", even though they're one character... so you'll have to dynamically size the grid by using the `wcwidth` function
	...
	* a student suggestion to get the test for passing (again not 💯 a requirement) to account for wcwidth when calculating max number of spaces
...
* `pieces` - an array of the "pieces" that are being player  (for example, ['😄', '🤮'])
...
* `error` - if any of the moves results in an error, this field is set to an object described below; otherwise, it should not be set at all (remain `undefined`)
    ...
    * example of the value stored in error: `{num: 8, val: '🤮', col: 'D'}`
...
⚠️ You can assume that if the `OPTIONS_AND_MOVES` commandline argument is supplied, __it will be in the valid format specified above__.
