...
2. implement functions below in __hoffy.js__
	* ⚠️  due to the structure of the tests for `compose`, `compose` must be present (even if just a function with an empty body) in the `hoffy.js` export
3. make sure you export your functions as you implement them so that...
4. you can run tests as you develop these functions (again, the tests are included in the repository, and `compose` must be xported as mention in an above step):
...
* `compareFunc` - a function used to determine if an element in `arr` is the one that we're looking for
	* has two parameters, `value1` and `value2` representing ele from `arr` and `num` 
...
* **Description:** This function returns an `Array` of objects containing the movie `title` and the `metascore`.  Note: return movies with `metascore` greater than 8.
* For example,
	```[
{"title":"Title 1","metascore":8.2}, {"title":"Title 2","metascore":8.1}
]```
