...
Your `auth.js` __requires the following implementation__:

* use `mongoose.model` to bring in your constructors
* __do not require your `db.js` file__; instead, bring in only `mongoose` 
...
* we haven't covered promises yet, but if you want to remove nesting, using promises is one solution (though you'll have to research this on your own): both `mongoose` and `bcrypt` provide promises (instead of callbacks) as part of their api
	* ⚠️ __if you do this, you cannot use the supplied unit tests__ 
	* instead, please add a note to a `README.md` file requesting the graders do a manual code review of your auth functions
